...
- **[2.1.14]** L'utente deve poter riordinare le carte in mano: per seme, per valore o manualmente.
- **[2.1.15]** L'utente deve poter vedere il numero di giocate e scarti rimanenti per il blind corrente.
- **[2.1.16]** L'utente deve poter vedere quali carte tra quelle giocate contribuiscono al punteggio.
- **[2.1.17]** L'utente deve poter vedere, al superamento di un blind o in caso di sconfitta, una schermata di riepilogo del blind con le seguenti informazioni: messaggio di superamento/sconfitta, punteggio richiesto dal blind, punteggio effettuato, numero di giocate rimanenti e numero di scarti rimanenti.
- **[2.1.18]** L'utente deve poter visualizzare le carte da gioco presenti nel mazzo in ordine.
- **[2.1.19]** L'utente deve poter visualizzare i livelli di ciascuna combinazione di carte.
- **[2.1.20]** L'utente deve poter selezionare al più un pacchetto dal negozio.
- **[2.1.21]** L'utente deve poter selezionare al più un oggetto da un pacchetto.
- **[2.1.22]** L'utente può specificare il seed della partita prima di avviarla (utilizzando la riga di comando).
- **[2.1.23]** L'utente deve poter avviare la ricerca di un seed (utilizzando la riga di comando) che soddisfa certi vincoli da lui espressi.
...
- **[2.2.9]** Le carte del mazzo possono formare le seguenti combinazioni, ognuna delle quali fornisce la base per il calcolo del punteggio:
  ...
  - _Four of a Kind_: 4 carte dello stesso valore (es. 4 assi). Punteggio di base: 60 Chips, 7 Mult;
...
- **[2.2.17]** Durante il round è possibile effettuare le seguenti azioni:
  ...
  - Cambiare l'ordine delle carte joker e nella mano.
- **[2.2.18]** Il calcolo del punteggio è suddiviso nelle seguenti fasi:
    1. Si considerano le carte selezionate e si determina il punteggio di base della giocata considerando la combinazione di carte e il corrispettivo livello. La combinazione di carte è scelta seguendo la priorità stabilita in **[2.2.9]**.
...
- **[2.2.19]** Una carta può venire _debuffed_. In questo caso la fase 3 e 4 del calcolo del punteggio ([2.2.18]) vengono saltate.
...
- **[2.2.26]** Le carte joker sono le seguenti:
  ...
  - _Onyx Agate_. Effetto "A carta a segno": +7 Mult se la carta è di Fiori;
