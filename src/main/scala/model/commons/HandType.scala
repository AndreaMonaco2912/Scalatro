...
object HandType:
  def contains(cards: Seq[Card], handType: HandType): Boolean =
    ...
    val isStraight = cards.map(_.rank.ordinal).sorted.sliding(2).forall { case Seq(a, b) => b - a == 1 }
    ...
    handType match
      ...
      case RoyalFlush => isStraight && isFlush && ranks.contains(Ace)
  ...
  def detect(cards: Seq[Card]): HandType =
    HandType.values.findLast(handType => contains(cards, handType)).getOrElse(HighCard)