...
object Score:
  ...
  def calculateHandScore(cards: Seq[Card])(using
    ...
    val jokerContext: JokerContext =
      JokerContext(cards, scoreConfig.levels, initialScore)
    val afterHandPlayed: JokerContext = jokers.foldLeft(jokerContext)(
      (acc, joker) => joker.onHandPlayed(cards).apply(acc)
    )
    val afterCards: JokerContext =
      scoringCards.foldLeft(afterHandPlayed)((acc, card) =>
        val cardContext = CardContext(acc.handScore)
        val afterCard: JokerContext =
          acc.copy(handScore = card.onScored(cardContext).handScore)
        jokers.foldLeft(afterCard)((acc, joker) =>
          joker.onCardScored(card).apply(acc)
        )
      )
    jokers
      .foldLeft(afterCards)((acc, joker) => joker.independent(acc))
      .handScore
