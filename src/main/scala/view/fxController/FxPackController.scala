...
class FxCardPackController extends FxPackController[Card]:
  override protected def renderItem(card: Card): Node = imageNode(Images.card(card))
  override protected def selectMsg(card: Card): PackSelection = PackSelection.SelectCard(card)

class FxPlanetPackController extends FxPackController[Planet]:
  override protected def renderItem(planet: Planet): Node = imageNode(Images.planet(planet))
  override protected def selectMsg(planet: Planet): PackSelection = PackSelection.SelectPlanet(planet)
...
class FxJokerPackController extends FxPackController[Joker]:
  override protected def renderItem(joker: Joker): Node =
    ...
    iv
  override protected def selectMsg(joker: Joker): PackSelection = PackSelection.SelectJoker(joker)